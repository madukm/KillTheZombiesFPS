...
class ClientConnector
{
    ...
    ClientConnector(unsigned short port, std::string address, Semaphore &_state_semaphore) 
    : state_semaphore(_state_semaphore)
    {
    ...
    void update()
    {
        ...
        while (true)
        {
            ...
            game_state_json = json("testing");//update_state.to_json(); //Must be in lock before reading and sending to server.
